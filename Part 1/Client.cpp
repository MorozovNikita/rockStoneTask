...
{
	this->io->stop();
	// Событие клиента, такое как disconnect может быть полезным логировать.
	// Это может позволить проанализировать статистику отключений.
...
void Client::params_set(const server::Packet &packet) const
{
	...
	// При обращении к полю player, я бы добавил проверку на существование, посколько 
	// это поле является указателем и может быть nullptr, а при обращении к нему в этом случае 
	// поведение программы будет неопределённым
	// if (!player)
	// {
...
void Client::buy(const server::Packet &packet)
{
	...
	// Аналогично комментарию стр. 68 - 74
...
void Client::login(const client::Packet *packet)
{
	...
	// В данном случае я бы использовал явный захват переменных.
	// При использовании символа &, лямбда получит доступ ко всем переменным из текущей области видимости,
	// а значения net_id и net_type и так уже сохранены в структуре data.
